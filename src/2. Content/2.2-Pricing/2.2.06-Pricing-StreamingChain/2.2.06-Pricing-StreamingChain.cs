...
namespace _2._2._06_Pricing_StreamingChain
{
    ...
    class Program
    {
        static void Main(string[] _)
        {
            try
            {
                ...
                using (ISession session = Configuration.Sessions.GetSession())
                {
                    ...
                    ProcessChain(StreamingChains.Definition(".AV.O").Streaming(true)
                                                                    .OnAdd((req, index, newv) => Console.WriteLine($"\tNew constituent {newv} added at index: {index}"))
                                                                    .OnRemove((req, index, oldv) => Console.WriteLine($"\tRemoved constituent {oldv} added at index: {index}"))
                                                                    .OnUpdate((req, index, oldv, newv) => Console.WriteLine($"Index {index} within our Chain changed from {oldv} => {newv}"))
                                                                    .OnStatus((req, item, status) => Console.WriteLine($"Status for item: {item} {status}"))
                                                                    .OnError((req, item, error) => Console.WriteLine($"Error for item: {item} {error}")));
                }
            }
            catch (Exception e)
            {
        ...
        private static void ProcessChain(IStreamingChains chain, bool changesEchoed = true)
        {
