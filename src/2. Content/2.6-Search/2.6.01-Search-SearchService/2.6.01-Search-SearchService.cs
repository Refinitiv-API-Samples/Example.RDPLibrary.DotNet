        ...
        static void Main(string[] _)
        {
            try
            {
                ...
                using (ISession session = Configuration.Sessions.GetSession())
                {
                    ...
                    // Search for Top CEOs in Apple - Display DocumentTitle and its subtype/components.
                    ...
                    DisplayResponse($"\nTop 3 CEOs in Apple:", request, "DTSubjectName", "DTSimpleType", "DTCharacteristics");
                    ...
                    var filter = "CouponRate gt 4 and RatingsScope(RatingType eq 'SPI' and CurrentRatingRank gt 15) and AssetState eq 'AC'";
                    request = Search.Definition(Search.View.GovCorpInstruments).Top(10)
                                                                               .Filter(filter)
                                                                               .Select("_, RatingsScope(filter:((RatingType xeq 'SPI'))), RatingX1XRatingRank");
                    DisplayResponse($"\n{filter}", request);
                }
            }
            catch (Exception e)
